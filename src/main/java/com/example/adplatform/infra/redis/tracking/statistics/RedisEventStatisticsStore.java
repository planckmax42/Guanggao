package com.example.adplatform.infra.redis.tracking.statistics;

import com.example.adplatform.infra.redis.delivery.DeliveryRedisKeys;
import com.example.adplatform.infra.redis.report.ReportRedisKeys;
...
public class RedisEventStatisticsStore implements EventStatisticsStore {
    ...
    public boolean recordEventOnce(EventMessage message, EventMaterialMetadata metadata) {
        ...
        Long result = stringRedisTemplate.execute(
                RECORD_EVENT_SCRIPT,
                List.of(
                        TrackingRedisKeys.eventStatisticsProcessed(message.eventId()),//事件去重第一步
                        statsKey,
                        ReportRedisKeys.dailyStatsDirtySet(localDate),
                        DeliveryRedisKeys.viewerPlanFrequency(
    ...
    public boolean recordCostOnce(
        ...
        Long result = stringRedisTemplate.execute(
                RECORD_COST_SCRIPT,
                List.of(
                        TrackingRedisKeys.eventCostStatisticsProcessed(message.eventId()),
                        statsKey(message, metadata, statDate),
                        ReportRedisKeys.dailyStatsDirtySet(statDate)),
                String.valueOf(DEDUP_TTL.toSeconds()),

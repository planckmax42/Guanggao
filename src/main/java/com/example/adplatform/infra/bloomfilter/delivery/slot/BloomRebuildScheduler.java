...
public class BloomRebuildScheduler {
    ...
    public void regularRebuild() {
        try {
            BloomRebuildResult rebuildResult = slotBloomOperationsService.regularRebuild();
    ...
    public void expandRebuildIfNeed() {
        BloomSnapshot bloomSnapshot = slotBloomOperationsService.getBloomSnapshot();
        double targetRate = bloomProperties.getFalsePositiveProbability();
        if (!bloomSnapshot.bloomFilterReady()||
                bloomSnapshot.actualFalsePositiveRate() < targetRate || bloomSnapshot.estimatedFalsePositiveRate() < targetRate) {
            return;
        }
        try {
            BloomRebuildResult rebuildResult = slotBloomOperationsService.expandRebuild();
            if(rebuildResult.rebuildStatus() == RebuildStatus.LOCK_BUSY){

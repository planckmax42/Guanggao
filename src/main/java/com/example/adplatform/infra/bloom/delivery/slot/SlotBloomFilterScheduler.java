...
public class SlotBloomFilterScheduler {
    ...
    public void checkAndExpand() {
        ...
        SlotBloomFilterService.BloomFilterSnapshot bloomFilterSnapshot =
                slotBloomFilterService.GetBloomFilterSnapshot();
        if (!bloomFilterSnapshot.bloomFilterReady()
        ...
        if (snapshot.actualFalsePositiveRate() < targetRate
                || bloomFilterSnapshot.expectedFpp() < targetRate) {
            return;
        }
        if (bloomFilterSnapshot.currentCapacity() >= slotBloomFilterProperties.getMaxExpectedCapacity()) {
            log.warn("广告位布隆过滤器已达到自动扩容上限，当前容量={}，实际误判率={}",
                    bloomFilterSnapshot.currentCapacity(), snapshot.actualFalsePositiveRate());
            return;
        }
        ...
        long oldCapacity = bloomFilterSnapshot.currentCapacity();
        if (slotBloomFilterService.expandRebuild()) {
            lastExpansionTime = Instant.now();
            SlotBloomFilterService.BloomFilterSnapshot expandedBloomFilterSnapshot =
                    slotBloomFilterService.GetBloomFilterSnapshot();
            log.warn("广告位布隆过滤器因误判率升高完成扩容，容量 {} -> {}，扩容前实际误判率={}，理论误判率={}",
                    oldCapacity,
                    expandedBloomFilterSnapshot.currentCapacity(),
                    snapshot.actualFalsePositiveRate(),
                    bloomFilterSnapshot.expectedFpp());
        }
    }
}

...
public interface OutboxMessageMapper {
    ...
    @Insert("""
            INSERT INTO outbox_message
                (event_id, topic, message_key, message_type, payload, bloomSnapshot, retry_count, next_retry_at)
            VALUES
    ...
    @Select("""
            SELECT id, event_id AS eventId, topic, message_key AS messageKey,
                   message_type AS messageType, payload, bloomSnapshot, retry_count AS retryCount,
                   next_retry_at AS nextRetryAt, created_at AS createdAt, sent_at AS sentAt
            FROM outbox_message
            WHERE bloomSnapshot = 'PENDING' AND next_retry_at <= NOW()
            ORDER BY id
            LIMIT #{limit}
    ...
    @Update("""
            ...
            SET bloomSnapshot = 'SENT', sent_at = NOW()
            WHERE id = #{id}
            """)
